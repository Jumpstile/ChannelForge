function Read-ChannelForgeRemoteM3UFetchCache {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$CacheRoot,
    ...
    $requiredMetadataFields = @(
        'CacheFormatVersion','CacheVersion','KeyAlgorithm','KeyVersion','CacheKey',
        'ProviderId','SourceId','Format','TransportContractVersion','ParserCacheVersion',
        'MaxDecompressedBytes','ContentType','ContentEncodings','PayloadFile',
        'PayloadByteCount','PayloadSha256','RawContentLength','ChannelCount',
        'FetchedAtUtc','ValidatedAtUtc','TtlSeconds','ETag','LastModified',
        'SourceHttpStatusCode','LastValidationStatus','OperationalReason')
    ...
    $unexpectedMetadataFields = @($metadata.PSObject.Properties.Name | Where-Object {
        $_ -notin $requiredMetadataFields
    })
    ...
    if ($null -ne $metadata.ContentType -and
        -not [string]::IsNullOrWhiteSpace([string]$metadata.ContentType) -and
        ([string]$metadata.ContentType).Trim().ToLowerInvariant() -notin $allowedContentTypes) {
    ...
    if ($payloadFile -notmatch '^payload-[0-9a-f]{64}\.m3u$') {
    ...
    if (-not [long]::TryParse([string]$metadata.PayloadByteCount, [ref]$payloadByteCount) -or
        $payloadByteCount -lt 0 -or $payloadByteCount -gt $MaxDocumentBytes) {

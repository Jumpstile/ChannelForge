class ChannelForgeRemoteM3UCacheTeeStream : System.IO.Stream {
    ...
    ChannelForgeRemoteM3UCacheTeeStream(
        [System.IO.Stream]$InnerStream,
        [string]$CachePath
    ) {
    ...
    [void] Flush() {
        if ($this.Disposed) {
            throw [System.ObjectDisposedException]::new('ChannelForgeRemoteM3UCacheTeeStream')
        }
        ...
        if ($read -gt 0) {
            try {
                ...
                $exception = [System.InvalidOperationException]::new(
                    'The remote M3U cache payload could not be written.',
                    $_.Exception)
    ...
    [void] Dispose([bool]$Disposing) {
        ...
        if ($Disposing) {
            ...
            try { $this.CacheStream.Dispose() } catch { }
            try { $this.HashAlgorithm.Dispose() } catch { }
        }

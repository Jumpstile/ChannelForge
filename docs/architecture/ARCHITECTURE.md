...
- **XMLTV generation and guide merge are deferred, not faked.** Local XMLTV import now provides programme and evidence objects, but the build pipeline does not yet consume them (`BuildContext.Programmes` remains an unused placeholder). Generating XMLTV without a merged guide would be fabricated content, which ADR 0005 (evidence over assumptions) does not allow. `build-summary.json` always reports `XMLTVGenerated: false` with an explicit `XMLTVDeferredReason`.

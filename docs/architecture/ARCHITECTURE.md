...
- **XMLTV generation is deferred, not faked.** There is no programme/guide data source anywhere in the repo (`BuildContext.Programmes` is an unused placeholder). Generating XMLTV without real programme data would be fabricated content, which ADR 0005 (evidence over assumptions) does not allow. `build-summary.json` always reports `XMLTVGenerated: false` with an explicit `XMLTVDeferredReason`.

...
RevisionContentId uses this contract's own hash discipline and no other scheme. RevisionContentManifest is UTF-8 text containing one line per normative file, in ascending file-name byte order, where each line is the lowercase 64-hex SHA-256 of that file's exact bytes, two ASCII spaces, the file name, and one LF. RevisionContentId is H(contract-revision-content/v1, RevisionContentManifest) using the H(D,B) formula defined in PART-A section 3.

contract-revision-content/v1 is a governance domain. It is not a semantic hash domain, it is not part of the exhaustive semantic domain inventory in PART-A section 3, it never appears in any semantic projection, and it identifies artifact revisions only.

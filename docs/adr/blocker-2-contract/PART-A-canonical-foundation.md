...
SelectedLogicalSourceIds is a required array of unique lowercase 64-hex LogicalSourceId strings sorted by ascending ordinal ASCII bytes; it is never null or omitted. InputArtifactHashes is a required array of ArtifactHashRecord objects sorted by LogicalSourceId ordinal bytes, then ArtifactKind rank, then ArtifactHash ordinal bytes; duplicate (LogicalSourceId, ArtifactKind) pairs are invalid. ArtifactHashRecord property order is LogicalSourceId, ArtifactKind, ArtifactHash. ArtifactKind is the closed enum M3U=1, XMLTV=2. For ArtifactKind=M3U, ArtifactHash is H(input-m3u/v2, exact input M3U file bytes). For ArtifactKind=XMLTV, ArtifactHash is H(input-xmltv/v2, exact input XMLTV file bytes). Exact input file bytes are the bytes supplied to the corresponding parser before parsing, decoding, newline conversion, normalization, trimming, transcoding, or canonical serialization. No pathname, URL, filesystem metadata, fetch timestamp, cache metadata, or source ordinal participates in ArtifactHash unless separately represented by another declared field. These mappings use the existing input-m3u/v2 and input-xmltv/v2 artifact-hash contracts and do not hash canonical candidate output bytes.

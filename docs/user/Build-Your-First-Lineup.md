...
- No channels in `merged.m3u` — check your source is `enabled: true` and has either a usable `local_playlist` or a supported remote `url`; an enabled source missing both fails the build closed instead of being silently skipped. Check `output/reports/build-summary.json` and `output/reports/lineup-plan.md` for the recorded failure.

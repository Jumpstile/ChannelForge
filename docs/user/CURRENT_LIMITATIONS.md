...
- A headless/server Guided Setup proposal contract for 1–8 playlists and 0–8 guides, with explicit selected/all guide bindings, no-guide proposals, server-owned staging, authenticated staged-byte hash/length verification, server-derived source identities, and durable enrollment of validated public HTTPS snapshots. Omitted multi-playlist guide bindings remain enrolled and actionable without being applied.
- Browser Guided Setup exposes the same source-set contract through `/api/guided-setup/proposal` and `/api/guided-setup/accept`: multiple local/public-HTTPS playlists, optional multiple guides, explicit selected/all bindings, and explicit no-guide mode. It requires acknowledgement, reuses immutable accepted-state publication, and leaves provider, downstream, and scheduler state unchanged.
...
- **Automatic refresh and publication remain deferred.** Browser and headless source enrollment support bounded public HTTPS acquisition, durable last-known-good snapshots, per-source manual refresh, explicit selected/all guide bindings, and browser multi-row setup. Refresh is manual and review-only: there is no unattended schedule, autonomous retry loop, or auto-accept. A changed source creates candidate/review evidence; a failed source preserves accepted state and the prior snapshot.

...
- **GUI Guided Setup remains preview-only for lineup operations.** The Tauri/React desktop shell includes the Workbench and a three-step Guided Setup layout for choosing a workspace, adding a playlist, and adding a guide. Its Rust-owned native picker bridge opens one item at a time in the order workspace → playlist → guide, performs pre-parse existence, expected-kind, and read-access checks, then performs bounded read-only structural scans and a single-guide exact identity comparison. Playlist scans accept UTF-8 with an optional BOM, require `#EXTM3U` as the first non-empty line, count complete channel entries, and return only safe status, reason code, and entry count. Guide scans accept UTF-8 with an optional BOM, require a `<tv>` root, count direct `<channel id="...">` and `<programme channel="..." start="..." stop="...">` entries, and return only safe status, reason code, channel count, and programme count; programme titles and guide channel IDs are not displayed. Guide files may be plain `.xml`/`.xmltv`, `.gz`, or a ZIP containing exactly one plain XML/XMLTV guide payload. Compressed input and final XMLTV output are each capped at 64 MiB; decompression is bounded, writes no extracted content to disk, and does not open URLs. Encrypted, empty, multi-guide, nested-compressed, and ambiguous ZIP contents fail closed. Playlist/guide matching compares only exact playlist `tvg-id` to XMLTV channel identity, reports matched, unmatched, ambiguous, and guide-only aggregate counts, and never automatically accepts an ambiguous relationship. React never receives paths, filenames, archive entry names, URLs, playlist stream lines, programme titles, guide channel IDs, credentials, or tokens. The GUI still does not match multiple guides, import or mutate a lineup, persist selections, export, or refresh a target.

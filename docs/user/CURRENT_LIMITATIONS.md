...
- **Scheduled refresh is manual-only.** The report-only planner and the manual foreground run wrapper can evaluate one eligible run and invoke the bounded source-refresh executor once under `output/operations/scheduled-refresh.lock`; there is still no live scheduler, daemon, service, worker, cron registration, or autonomous retry loop.

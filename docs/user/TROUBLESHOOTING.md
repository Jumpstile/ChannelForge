...
There is a Tauri/React desktop shell with a native picker bridge that checks pre-parse availability, expected kind, and read access. A selected M3U playlist and local XMLTV guide also receive bounded structural checks; the UI reports only safe status, reason, and aggregate counts. Guide checks support plain `.xml`/`.xmltv`, `.gz`, and a ZIP containing exactly one plain XML/XMLTV payload, but do not write extracted content or decompress nested guide payloads. It never opens or displays stream URLs, programme titles, or guide channel IDs, and it does not match playlist and guide identities, build lineups, save changes, export, or update providers. If content needs attention, use the displayed reason without sharing the file or URL. See [Current Limitations](CURRENT_LIMITATIONS.md).

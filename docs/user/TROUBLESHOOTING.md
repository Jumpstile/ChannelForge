...
| No channels in `merged.m3u`, or fewer than expected        | An enabled source is missing both a usable `local_playlist` and a supported remote `url`                             | The build fails closed instead of silently skipping that source — check `build-summary.json` and `lineup-plan.md` for the recorded failure and correct the source configuration |

...
- **No automatic target-specific guide assignment.** ChannelForge reports exact M3U/XMLTV identity bindings and keeps the separate XMLTV output canonical; Plex will not consume that guide output automatically, so configure the guide path explicitly when using a local or remote XMLTV source. Remote XMLTV acquisition remains bounded HTTPS/443.

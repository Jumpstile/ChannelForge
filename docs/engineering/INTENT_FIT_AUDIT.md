...
| GUI saved-lineup flow and validation              | Preserve reusable Guided Setup, selection, validation/review, saved-lineup, accessibility, copy, and design-token work while moving the primary product shell to a browser web UI; keep state changes behind the engine/API and immutable acceptance boundary.               | Future web UI engine/API entry points; current `GuidedSetupPage`, `LineupReviewPage`, `SavedLineupPage`; optional Tauri `prepare_saved_lineup_plan` and `accept_saved_lineup` adapters       | `gui/src/pages/GuidedSetupPage.tsx`; `gui/src/pages/LineupReviewPage.tsx`; `gui/src/pages/SavedLineupPage.tsx`; `gui/src/app/setupSelection.ts`; `gui/src/app/setupPicker.ts`; `gui/src-tauri/src/lib.rs`; GUI unit/e2e tests; Issue #150 architecture docs | Existing GUI tests and Rust bridge preserve blocked review states, explicit acknowledgement, redaction, and PowerShell-owned acceptance. Web migration is not implemented in this slice.                             | Web HTTP/API entry points and native-assumption migration remain future work; Issue #145 remains a CI gap for GUI checks.                                                                    | PASS_WITH_GAPS |
...
| CI, build, and test correctness gates             | Run formatting, schema, documentation, analyzer, secret, and PowerShell test gates before merge; frame future GUI coverage as web UI TypeScript/Vitest first, with Tauri/Rust checks only for the optional wrapper, and keep review evidence distinct from release approval. | `.github/workflows/powershell-ci.yml`, future GUI workflow, validation scripts, package scripts                                                                                              | `.github/workflows/powershell-ci.yml`; `scripts/Validate-*`; `package.json`; `tests/unit/*`; `gui/package.json`; Issue #145; Issue #150 ADR and architecture docs                                                                                           | CI #225 / run `34898913886` passed for the exact base. The current workflow still does not execute GUI TypeScript, Vitest, or Rust/Tauri checks, so #145 remains a follow-up.                                        | GUI CI coverage and web/API checks are not yet wired.                                                                                                                                        | PASS_WITH_GAPS |
...
The GUI area remains `PASS_WITH_GAPS` because Issue #145's TypeScript, Vitest,
and optional Tauri/Rust checks are not yet in repository CI.

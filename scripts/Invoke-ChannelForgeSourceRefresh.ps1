...
Import-Module (Join-Path $moduleRoot 'src/ChannelForge/ChannelForge.psd1') -Force
 ...
 $plan = @(Get-ChannelForgeSourceRefreshPlan -ProviderConfigPath $ProviderConfigPath -EpgConfigPath $EpgConfigPath -CacheRoot $CacheRoot -EvaluationTimeUtc $EvaluationTimeUtc) | Where-Object { $_.PSObject.Properties.Name -contains 'RecommendedAction' }
 $rows = [System.Collections.Generic.List[object]]::new()

foreach ($planned in $plan) {
    $source = if ($planned.Kind -eq 'remote' -and $planned.SourceId.StartsWith('m3u-')) { $providerConfig | Where-Object Name -eq $planned.Name | Select-Object -First 1 } else { $epgConfig | Where-Object Name -eq $planned.Name | Select-Object -First 1 }
    $row = [ordered]@{ SourceId=[string]$planned.SourceId; Name=[string]$planned.Name; Kind=[string]$planned.Kind; PlannedAction=[string]$planned.RecommendedAction; Attempted=$false; Result='NOT_ATTEMPTED'; CacheChanged=$false; LastKnownGoodPreserved=$false; ValidatorOutcome=[string]$planned.Validator; SafeReason=[string]$planned.Reason }
    if ($planned.RecommendedAction -in @('CONDITIONAL_REFRESH','FULL_REFRESH')) {
        $row.Attempted = $true
        $status = [ordered]@{}
        try {
            if ($planned.SourceId.StartsWith('m3u-')) { @(Import-ChannelForgeConfiguredM3USource -Source $source -Provider $providerId -CacheRoot $CacheRoot -EvaluationTimeUtc $EvaluationTimeUtc -AcquisitionStatus $status) | Out-Null }
            ...
            $row.SafeReason = if ($row.LastKnownGoodPreserved) { 'Refresh failed; last-known-good cache was preserved.' } else { 'Refresh failed; no validated cache was available to preserve.' }
        }
...
$projection = [ordered]@{ SchemaVersion='source-refresh-result/v1'; EvaluationTimeUtc=$EvaluationTimeUtc.ToUniversalTime().ToString('o'); Sources=@($rows | Sort-Object Kind,SourceId) }
$reportRoot = [IO.Path]::GetFullPath($OutputRoot); New-Item -ItemType Directory -Force -Path $reportRoot | Out-Null
$jsonPath = Join-Path $reportRoot 'source-refresh-result.json'; $mdPath = Join-Path $reportRoot 'source-refresh-result.md'
[IO.File]::WriteAllText($jsonPath, (($projection | ConvertTo-Json -Depth 8) + [Environment]::NewLine), [Text.UTF8Encoding]::new($false))
$md = [System.Collections.Generic.List[string]]::new(); $md.Add('# Source refresh result'); $md.Add(''); $md.Add("Evaluation time: $($projection.EvaluationTimeUtc)"); $md.Add('This one-shot operation refreshes disposable source cache evidence only. It does not publish or change accepted output.'); $md.Add(''); $md.Add('| Source | Kind | Planned action | Result | Cache changed | Last-known-good preserved |'); $md.Add('| --- | --- | --- | --- | --- | --- |'); foreach ($s in $projection.Sources) { $md.Add("| $($s.Name) | $($s.Kind) | $($s.PlannedAction) | $($s.Result) | $($s.CacheChanged) | $($s.LastKnownGoodPreserved) |") }; $md.Add(''); $md.Add('A failed refresh never replaces a previously validated cache.'); [IO.File]::WriteAllText($mdPath, (($md -join [Environment]::NewLine) + [Environment]::NewLine), [Text.UTF8Encoding]::new($false))
[pscustomobject][ordered]@{ JsonPath=$jsonPath; MarkdownPath=$mdPath; SourceCount=$projection.Sources.Count }
